...
const Projects = () => {
      ...
      const [projects, setProjects] = useState([
            ...
            {title: 'Optical Character Recognition',
                  desc: 'This research investigated the use of feature matching and template matching for optical character recognition of pre-segmented typed Arabic characters Optical character recognition (OCR) is an automation process which allows the conversion from written characters to text documents representing the characters. Text files use less memory and are more easily processed for data mining such as use in a translation memory compared to image files. Automation of the process, while computationally-demanding, allows more efficient use of limited human resources. The primary method explored utilized stochastic analysis of various measured characteristics of each character. Work was also done using k-nearest neighbor and multi-layer perceptron classification methods. Within MATLAB, .bmp files were converted into binary matrices. These images were degraded using Gaussian noise, JPEG compression, rotation, and shrinking the images. Data corresponding to various aspects of the characters was collected and processed using an existing algorithm. Accuracy varied depending upon the learning set and test set used.',
                  photo: {ocr}, code: '', live: '', tech: ['matlab', 'terminal', 'ml']},
            {title: 'Tic-tac-toe',
      ...
      return (<>
            <div className='projects'>
                  <h2>Projects</h2>
                  <div className='track'>
                        {projects.map((work, index) => <Work title={work.title} desc={work.desc} photo={work.photo} code={work.code} tech={work.tech} live={work.live} index={index}/>)}
                  </div>
            </div>
      </>)
}
